...
/* Haalt een tijd (HH:MM) uit een gecombineerde datum/tijd-waarde.
   Werkt voor "27-06-2026 15:58", "06/27/2026 15:58", ISO-strings, enz.
   Geeft null als er geen tijd in zit (dan blijft de kolom "Tijd" leeg). */
...
/* Haalt de TIJD (HH:MM) uit een Meta-Publicatietijdstip.
   In de Facebook-export staat dit als tekst, bv. "06/08/2026 07:15"
   (MM/DD/YYYY HH:MM). De tijd werd voorheen genegeerd, waardoor de
   kolom "Tijd" in het dashboard leeg (undefined) bleef. */
function parseMetaTime(s) {
...
export function parseMetaFacebookSheet(rows) {
  ...
  // Saves worden door Meta niet standaard meegeleverd. De gebruiker mag
  // zelf een kolom toevoegen; we herkennen meerdere gangbare benamingen
  // (case-insensitief). Ontbreekt de kolom, dan blijft saves 'null'
  // (= onbekend), zodat achterwaartse compatibiliteit behouden blijft.
  const savesAliases = ['saves', 'opgeslagen', 'bewaard', 'bewaringen', 'opslaan'];
  const savesIdx = H.findIndex((h) => savesAliases.includes(String(h || '').trim().toLowerCase()));
  const idx = {
  ...
  for (let i = hIdx + 1; i < rows.length; i++) {
    ...
    if (!date) continue;
    // Tijd uit het Publicatietijdstip (datum + tijd staan samen in die cel).
    let time = (idx.pub > -1) ? parseMetaTime(r[idx.pub]) : null;
    if (!time && idx.datum > -1) time = parseMetaTime(r[idx.datum]);
    ...
    out.push({
      ...
      saves: idx.saves > -1 ? parseNumber(r[idx.saves]) : null,
      follows: null,
      watchDuration: watch,
