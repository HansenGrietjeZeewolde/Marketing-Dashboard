...
2. Settings → Environment Variables (zie `.env.example`):
   - `SUPABASE_URL` (Production + Preview + Development)
   - `SUPABASE_ANON_KEY`
   - `SUPABASE_SERVICE_ROLE_KEY` — **alleen** hier, server-side.
3. Deploy. De build draait `scripts/build-env.js` en de functie `/api/users` komt online.
